...
def calcElevationCost(next_X, next_Y, curr_X, curr_Y, M, mtrx):
    if abs(mtrx[next_X][next_Y]) - abs(mtrx[curr_X][curr_Y]) <= M:
        return 0
    else:
        return max(0, abs(mtrx[next_X][next_Y]) - abs(mtrx[curr_X][curr_Y]) - M)

def expandAS(X, Y, mtrx, height, width, stamina, visited, open_queue, open_queue_list, parent_cost, lodge, parent):
    step_cost = 10
    print("stamina", stamina)

    M = calcMomemtum(parent[str(X) + str(Y)][0], parent[str(X) + str(Y)][1], X, Y, mtrx)
    
    if allowedMoves(M, X, Y, X-1, Y, mtrx, stamina, height, width):   # North
        
        if not isVisited(open_queue_list, X-1, Y):
            open_queue.put((parent_cost + heuristic(X-1, Y, lodge[1], lodge[0]) + step_cost + calcElevationCost(X-1, Y, X, Y, M, mtrx), parent_cost + step_cost, (X-1, Y)))
    ...
    if allowedMoves(M, X, Y, X, Y+1, mtrx, stamina, height, width):   # East
        if not isVisited(open_queue_list, X, Y+1):
            open_queue.put((parent_cost + heuristic(X, Y+1, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X, Y+1, X, Y, M, mtrx), (X, Y+1)))
    ...
    if allowedMoves(M, X, Y, X+1, Y, mtrx, stamina, height, width):   # South
        if not isVisited(open_queue_list, X+1, Y):
            open_queue.put((parent_cost + heuristic(X+1, Y, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X+1, Y, X, Y, M, mtrx), (X+1, Y)))
            open_queue_list[str(X+1) + str(Y)] = parent_cost + step_cost
            parent[str(X+1) + str(Y)] = str(X) + str(Y)
        elif open_queue_list[str(X+1) + str(Y)] > parent_cost + step_cost:
            removeFromopen_queue(open_queue, X+1, Y)
    ...
    if allowedMoves(M, X, Y, X, Y-1, mtrx, stamina, height, width):   # West
        if not isVisited(open_queue_list, X, Y-1):
            open_queue.put((parent_cost + heuristic(X, Y-1, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X, Y-1, X, Y, M, mtrx), (X, Y-1)))
            open_queue_list[str(X) + str(Y-1)] = parent_cost + step_cost
            parent[str(X) + str(Y-1)] = str(X) + str(Y)
    ...
    step_cost = 14
    if allowedMoves(M, X, Y, X-1, Y+1, mtrx, stamina, height, width):   # NorthEast
        if not isVisited(open_queue_list, X-1, Y+1):
            open_queue.put((parent_cost + heuristic(X-1, Y+1, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X-1, Y+1, X, Y, M, mtrx), (X-1, Y+1)))
    ...
    if allowedMoves(M, X, Y, X+1, Y+1, mtrx, stamina, height, width):   # SouthEast
        if not isVisited(open_queue_list, X+1, Y+1):
            open_queue.put((parent_cost + heuristic(X+1, Y+1, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X+1, Y+1, X, Y, M, mtrx), (X+1, Y+1)))
            open_queue_list[str(X+1) + str(Y+1)] = parent_cost + step_cost
            parent[str(X+1) + str(Y+1)] = str(X) + str(Y)
    ...
    if allowedMoves(M, X, Y, X+1, Y-1, mtrx, stamina, height, width):   # SouthWest
        if not isVisited(open_queue_list, X+1, Y-1):
            open_queue.put((parent_cost + heuristic(X+1, Y-1, lodge[1], lodge[0]) + step_cost, parent_cost + step_cost + calcElevationCost(X+1, Y-1, X, Y, M, mtrx), (X+1, Y-1)))
            open_queue_list[str(X+1) + str(Y-1)] = parent_cost + step_cost
            parent[str(X+1) + str(Y-1)] = str(X) + str(Y)
